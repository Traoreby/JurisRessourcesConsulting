    ...
    <div className="py-20 bg-background min-h-screen">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
          {loading ? (
            <div className="col-span-full text-center py-10 text-slate-500">Chargement des actualités...</div>
          ) : articles.length === 0 ? (
            <div className="col-span-full text-center py-10 text-slate-500">Aucune actualité disponible pour le moment.</div>
            ...
            >
              <div className="relative h-56 overflow-hidden">
                ...
                {article.image ? (
                  <img src={article.image} alt={article.titre} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                ) : (
              ...
              <div className="p-8 flex-grow flex flex-col">
                ...
                <div className="flex items-center gap-6 text-sm text-slate-600 mb-6 font-medium">
                  <div className="flex items-center gap-2"><Calendar size={16} className="text-accent" aria-hidden="true" /> {new Date(article.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
                  <div className="flex items-center gap-2"><User size={16} className="text-accent" aria-hidden="true" /> {article.auteur}</div>
                </div>
